...
**Capture is on by default.** Whenever the proxy is enabled, `--body`/`include=body` work with no extra config — headers and bodies are recorded automatically. Sensitive header values (`Authorization`, `Cookie`, `X-Api-Key`, etc.) and sensitive query-param values (`token`, `api_key`, `code`, etc., including inside `Location`/`Referer` redirects) show up as `[REDACTED]`/`REDACTED` rather than the real value — that marker means the field was present, not that it's missing. **Bodies are NOT redacted** — request/response bodies are stored verbatim, so treat any JSON/form body you pull with `--body` as potentially containing real secrets. A project can opt out entirely with `proxy.capture.enabled: false` in `prox.yaml` or `prox up --no-capture` for one run; when that's the case (or a record is metadata-only, e.g. a WebSocket upgrade or routing error), `prox requests <id>` prints a hint explaining why there's no captured detail instead of the body. `prox proxy status` reports `capture_available`/`capture_error` (daemon-wide: capture failed to initialize) and a used/budget disk line — check it if bodies are unexpectedly missing on a shared daemon. See `references/configuration.md` for the full field list and the disk-budget/eviction rule.
