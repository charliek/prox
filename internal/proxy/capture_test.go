...
func TestCaptureRequest(t *testing.T) {
	t.Run("disabled manager returns original body unchanged", func(t *testing.T) {
		...
		capturedBody, wrappedBody, headers := cm.CaptureRequest("req1", req, 0)
		assert.Nil(t, capturedBody)
		assert.Equal(t, body, wrappedBody) // same object
		assert.Equal(t, "text/plain", headers.Get("Content-Type"))
	})

	t.Run("nil body returns nil captured body and verbatim headers", func(t *testing.T) {
		...
		// A bodyless request still reaches the stored Details through this header
		// clone, and its sensitive values are kept verbatim.
		require.NotNil(t, headers)
		assert.Equal(t, []string{"Bearer topsecret"}, headers["Authorization"])
		assert.Equal(t, []string{"session=SECRETSESSION"}, headers["Cookie"])
		// The returned map is a clone, not an alias of the live request headers.
		headers.Set("Authorization", "mutated")
		assert.Equal(t, []string{"Bearer topsecret"}, req.Header["Authorization"])
	})
	...
	t.Run("large body is written to disk", func(t *testing.T) {
		...
		capturedBody, wrappedBody, _ := cm.CaptureRequest("req-large", req, 0)
	...
	t.Run("body exceeding max size is truncated", func(t *testing.T) {
		...
		capturedBody, wrappedBody, _ := cm.CaptureRequest("req-trunc", req, 0)
	...
	t.Run("headers are cloned independently", func(t *testing.T) {
		...
		_, wrappedBody, headers := cm.CaptureRequest("req1", req, 0)
...
func TestFinalizeRequestBody(t *testing.T) {
	t.Run("forces finalize on a wrapped body before Close", func(t *testing.T) {
		cm := newEnabledCaptureManager(t)
		req := httptest.NewRequest("POST", "/test", strings.NewReader("payload"))
		capturedBody, wrappedBody, _ := cm.CaptureRequest("req-force", req, 0)
...
func TestContentEncodingCaptured(t *testing.T) {
	t.Run("request body records Content-Encoding", func(t *testing.T) {
		...
		capturedBody, wrappedBody, _ := cm.CaptureRequest("req-enc", req, 0)
		require.NotNil(t, capturedBody)
	...
	t.Run("absent Content-Encoding stays empty", func(t *testing.T) {
		...
		req := httptest.NewRequest("POST", "/test", strings.NewReader("payload"))
		capturedBody, wrappedBody, _ := cm.CaptureRequest("req-noenc", req, 0)
		require.NotNil(t, capturedBody)
		_, err := io.ReadAll(wrappedBody)
		require.NoError(t, err)
		require.NoError(t, wrappedBody.Close())
...
// TestPerCallCaptureLimits pins the D13 per-call caps: CaptureRequest and
// WrapResponseWriter honor a positive maxBodySize argument, while a 0 falls back
// to the manager's configured cap (which itself defaults to
...
func TestPerCallCaptureLimits(t *testing.T) {
	...
	t.Run("request tighter per-call limit wins", func(t *testing.T) {
		req := httptest.NewRequest("POST", "/test", strings.NewReader(payload))
		captured, wrapped, _ := cm.CaptureRequest("req-tight", req, 10)
	...
	t.Run("request looser per-call limit wins", func(t *testing.T) {
		req := httptest.NewRequest("POST", "/test", strings.NewReader(payload))
		captured, wrapped, _ := cm.CaptureRequest("req-loose", req, 150)
		_, _ = io.ReadAll(wrapped)
		require.NoError(t, wrapped.Close())
	...
	t.Run("request zero per-call limit falls back to manager cap", func(t *testing.T) {
		req := httptest.NewRequest("POST", "/test", strings.NewReader(payload))
		captured, wrapped, _ := cm.CaptureRequest("req-zero", req, 0)
		_, _ = io.ReadAll(wrapped)
		require.NoError(t, wrapped.Close())
	...
	t.Run("response per-call limit and zero fallback", func(t *testing.T) {
		...
		tight := cm.WrapResponseWriter(httptest.NewRecorder(), 10)
		...
		fallback := cm.WrapResponseWriter(httptest.NewRecorder(), 0)
