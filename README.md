...
- **Remote proxy hub** - Publish a project's services through a `prox` daemon on another machine, so a hostname like `auth.llt.example.com` works from a phone, a teammate's laptop, or a VM with only outbound network access ([guide](docs/guides/remote-hub.md))
