...
- **Per-project caps, one capture path.** The shared daemon has a single `CaptureManager` and capture directory, but each project's `max_body_size` (sent at registration) is enforced per call via a policy-taking capture API (`CaptureRequest(requestID, r, policy)`/`WrapResponseWriter(w, policy)`) rather than one `CaptureManager` per project — one shared cleanup path, per-project ceilings.
- **Disk accountant and budget enforcement (#69).** A `diskAccountant` is the single chokepoint every spilled body file (write or delete) passes through, tracking per-record-group bytes (`_req`/`_res` age together, keyed by whichever spilled first). Enforcement happens synchronously inside `store()` right after a write is accounted, so a budget breach is corrected before the next spill rather than on a timer: it evicts the oldest group(s) — FIFO by first-spill time, tie-broken by a monotonic sequence, deliberately never LRU — until usage is back at or under budget. The effective daemon-wide budget is the minimum, across every capture-enabled project, of that project's `disk_budget` (default 1GiB); `Registry.EffectiveCaptureDiskBudget()` recomputes it and `Server.syncCaptureBudget()` re-pushes it into the shared `CaptureManager` after every committed registry mutation (register/rollback/restore/deregister/stale-removal). `GET /status` reads used/budget as one snapshot (`CaptureManager.DiskStats()`) so the pair is never torn across an eviction.
- **Capture-time redaction (plan 012 D4).** A per-call `CapturePolicy` (built from each project's `CaptureConfig` and threaded through the daemon's register wire, deep-copied per route) redacts sensitive header values (`Authorization`, `Cookie`, etc. → `[REDACTED]`) and sensitive query-param values (`token`, `api_key`, etc. → `REDACTED`) **at the moment of capture** — before disk, the daemon→project SSE bridge, the API, or the TUI ever see the record. Redaction also rewrites query params inside `Location`/`Referer` header values and URL fragments (via string surgery, fail-closed against unparseable values) and userinfo passwords in URLs. It never mutates the upstream request/response. Bodies are explicitly out of scope — a v1 limitation documented in the [Request Capture](../reference/configuration.md#request-capture) reference.
