...
- **Per-project caps, one capture path.** The shared daemon has a single `CaptureManager` and capture directory, but each project's `max_body_size` (sent at registration) is enforced per call via a plain `maxBodySize int64` parameter (`CaptureRequest(requestID, r, maxBodySize)`/`WrapResponseWriter(w, maxBodySize)`) rather than one `CaptureManager` per project — one shared cleanup path, per-project ceilings.
...
- **Cleartext capture.** Captured records — request URLs, request/response headers, and bodies up to `max_body_size` — are stored exactly as sent, with no transformation applied to sensitive values. See the [Request Capture](../reference/configuration.md#request-capture) reference for the full cleartext posture and the private (`0700`/`0600`) file permissions that back it.
